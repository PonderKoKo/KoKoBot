...
const { dataFileName, threeCardFileName, recordsFileName, imagesPath, prefix, token, timeBetweenRefresh, scryfallRequestDelayTime, isTest } = require('./config.json');
...
client.once('ready', () => {
	...
	readFile(dataFileName, function(err, content) {
		if (err) {
			process.exit(1);
		}
	...
	readFile(threeCardFileName, function (err, content) {
		module.exports.decks = JSON.parse(content);
		readFile(recordsFileName, function (err2, content2) {
...
client.on('guildMemberAdd', member => {
	...
	client.channels.fetch('626215981387350059')
		.then(channel => {
			/*
			if (member.guild.memberCount % 200 === 0) {
				channel.send(`Congratulations, <@${member.user.id}>, you are member number ${String(member.guild.memberCount)}! You win nothing.`);
			}
			else {
				channel.send(`Welcome to the server, <@${member.user.id}>. We are now ${String(200 - (member.guild.memberCount % 200))} members away from ${String(200 * (Math.floor(member.guild.memberCount / 200) + 1))}.\n**Please read <#626217085743136778> and <#645407379151781899> before posting!**`);
			}
			*/
			channel.send(`Welcome to the server, <@${member.user.id}>. We have both a Custom Card Creation and a 3-Card-Magic contest running right now! Check out <#645407379151781899> if you're interested.`);
...
function scryfallAndBackup() {
	...
	for (let i = 0; i < ALLSETS.length; i++) {
		...
		if (spoilerData.sets[setkey].isWatched) client.setTimeout(scryfallRequest, scryfallRequestDelayTime * i, spoilerData.sets[setkey].CODE);
	}
...
function addSet(setcode, name, channel) {
	...
	if (!Object.keys(spoilerData.sets).includes(SETKEY)) {
		spoilerData.sets[SETKEY] = { 'isWatched': false, 'CODE': setcode.toUpperCase(), 'name': name, 'channelIDs': spoilerData.autowatch, 'cardIDs': {} };
